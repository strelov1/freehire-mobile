...
export default function FiltersScreen() {
  ...
  // Hidden only while the profile fetch is in flight, so the button doesn't
  // change meaning under the user's finger. A signed-out visitor and a
  // signed-in one with no saved profile both see it: there is somewhere to
  // send each of them now — sign-in and the profile editor respectively.
  const showApplyProfile = !user || !profileLoading;
        ...
        <View style={styles.headerActions}>
          {showApplyProfile ? (
            <Pressable
              ...
              {/* A signed-in user with no profile is offered the editor, so the
                  button says so — "Apply profile" would name something that
                  isn't there yet. */}
              <Text style={[styles.applyProfileText, { color: c.brandForeground }]}>
                {user && !profile ? 'Set up profile' : 'Apply profile'}
              </Text>
            </Pressable>
          ) : null}
          <Pressable
