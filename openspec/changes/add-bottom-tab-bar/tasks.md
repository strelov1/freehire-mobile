...
- [x] 1.2 Implement `nextTabBarHidden` until tests pass.

## 2. Shared tab-bar-visibility context

- [x] 2.1 Add `src/lib/tabBarVisibility.tsx` (or extend the file above): a `TabBarVisibilityProvider` + `useTabBarVisibility()` exposing a Reanimated `SharedValue<number>` (`translateY`, 0=shown) and a plain JS setter that (a) tracks last scroll offset, (b) calls `nextTabBarHidden`, (c) animates the shared value via `withTiming` when the boolean flips.
- [x] 2.2 Mount `TabBarVisibilityProvider` in `src/app/_layout.tsx` alongside `FilterProvider`.

## 3. Route restructuring into `(tabs)`

- [x] 3.1 Create `src/app/(tabs)/` and move `src/app/index.tsx` into
...
- [x] 3.2 Move `src/app/notifications.tsx` into
...
- [x] 3.3 Move `src/app/profile.tsx` into `src/app/(tabs)/profile.tsx`: drop
...
- [x] 3.4 Create `src/app/(tabs)/companies.tsx`: a static centered
      icon + "Companies — coming soon" screen, no data fetching, styled
      with existing `Space`/`Radius`/color tokens.
- [x] 3.5 Delete the now-empty root `src/app/notifications.tsx` and
...
- [x] 4.1 Create `src/app/(tabs)/_layout.tsx`: a `Tabs` navigator with
...
- [x] 4.2 Implement the custom tab bar component (co-located in
      ...
      tab-bar contract.
- [x] 4.3 Add bottom padding (matching the tab bar's height) to each tab
      screen's scrollable content so the bar doesn't initially cover the
      last item.

## 5. Root layout + call-site updates

- [x] 5.1 In `src/app/_layout.tsx`, replace the root `Stack.Screen
...
- [x] 5.2 Grep for and update any remaining `router.push('/notifications')`
...
- [x] 6.1 Run unit tests (`tabBarVisibility.test.ts`) and `tsc --noEmit`.
- [x] 6.2 Run `npm run lint`.
- [x] 6.3 Verify end-to-end in the iOS simulator against every scenario in
      ...
      feed.
- [x] 6.4 Run the `simplify` pass over the changed files, then request code
      review.
